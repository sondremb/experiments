...
export type DensityFunction = (point: Point) => number;

export function variableDensityPoisson(
	minX: number,
	maxX: number,
	minY: number,
	maxY: number,
	...
	function addSample(sample: Point) {
		...
		backgroundGrid[Math.floor((sample.x - minX) / gridSize)][
			Math.floor((sample.y - minY) / gridSize)
		] = sampleIndex;
	}

	function isLegalSample(sample: Point) {
		if (
			sample.x < minX ||
			sample.x > maxX ||
			sample.y < minY ||
		...
		for (
			let x = Math.max(gridX - maxGridDistance, 0);
			x <= Math.min(gridX + maxGridDistance, backgroundGrid.length - 1);
			x++
		) {
			for (
				let y = Math.max(gridY - maxGridDistance, 0);
				y <= Math.min(gridY + maxGridDistance, backgroundGrid[0].length);
				y++
			) {
